        ...
        readdir(path, function (err, children) {
            if (err) {
                ...
                pending.unshift.apply(pending, tslib_1.__spread(children
                    ...
                    .map(function (child) { return join(path, child); })));
            }
            processNext();
        });
    }
    function processFile(path) {
        var extension = path.endsWith('.coffee.md') ? '.coffee.md' : extname(path);
        var outputPath = join(dirname(path), basename(path, extension)) + '.js';
        ...
        readFile(path, 'utf8', function (err, data) {
            ...
            writeFile(outputPath, resultCode, function (err) {
                if (err) {
                    errors.push(err);
                }
                processNext();
