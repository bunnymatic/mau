  ...
  describe 'favorites -'  do
    ...
    describe "mailer notifications" do
      ...
      it '#resend_activation sends a new activation email' do
        expect(UserMailer).to receive('resend_activation').with(maufan).once.and_return(double('deliverable', :deliver => true))
        maufan.resend_activation
      end
      it '#create_reset_code sends a recent reset email' do
        expect(UserMailer).to receive('reset_notification').with(maufan).once.and_return(double('deliverable', :deliver => true))
        maufan.create_reset_code
      end

      it "add art_piece favorite sends favorite notification to owner" do
        expect(ArtistMailer).to receive('favorite_notification').with(artist, maufan).once.and_return(double('deliverable', :deliver => true))
