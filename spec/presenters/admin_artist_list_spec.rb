...
describe AdminArtistList, elasticsearch: false do
  ...
  let(:active_artists) do
    [FactoryBot.create(:artist, :active, updated_at: 1.month.ago)]
  end
  ...
  let!(:artists) do
    (active_artists + bad_standing_artists + pending_artists).sort_by(&:lastname)
  end
    ...
    describe 'when there is a current open studios with participants' do
      ...
      let(:artists) do
        Time.use_zone(Conf.event_time_zone) do
          os_event
          artist_list = [
            create(:artist, :active, :with_art, :with_phone, doing_open_studios: true, updated_at: 5.days.ago, last_request_at: 2.days.ago),
            create(:artist, :active, :with_art, :with_phone, doing_open_studios: true, updated_at: 4.days.ago, last_request_at: 2.days.ago),
          ]
          artist_list.sort_by!(&:updated_at)
