  ...
  describe('mau.services.favoriteService', function() {
    ...
    return describe('.add', function() {
      it("calls the right endpoint to add this favorite if there is a logged in user", function() {
        ...
        this.http.expect('POST', '/users/the_user/favorites', expectedPost).respond(success);
      ...
      return it("returns a message if there is favoriting fails", function() {
        ...
        this.http.expect('POST', '/users/somebody/favorites', expectedPost).respond(500, {});
        response = this.service.add(null, null);
        this.http.flush();
        return response.then(function(data) {
          return expect(data).toEqual({
            message: "That item doesn't seem to be available to favorite.  If you think it should be, please drop us a note and we'll look into it."
          });
        });
      });
    });
