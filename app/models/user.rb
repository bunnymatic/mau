...
class User < ActiveRecord::Base
  ...
  def notify_favorited_user(fav)
    ...
    if artist && artist.emailsettings['favorites']
      ArtistMailer.favorite_notification(artist, self).deliver_now
    end
  end
  ...
  def tell_user_they_signed_up
    if is_artist?
      reload
      ArtistMailer.signup_notification(self).deliver_now
    end
  end
  ...
  def notify_user_about_state_change
    ...
    mailer_class.reset_notification(self).deliver_now if recently_reset?
    mailer_class.resend_activation(self).deliver_now if resent_activation?
  end
