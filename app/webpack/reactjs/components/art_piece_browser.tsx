...
export const ArtPieceBrowser:FC<ArtPieceBrowserProps> = ({artistId, artPieceId}) => {
  ...
  useEffect(() => {
    ...
    const artPiecesCall = api.artPieces
                             .index(artistId)
    Promise.all([artistCall, artPiecesCall]).then(([artist, artPieces]) => {
      setArtist(artist);
      setArtPieces(artPieces);
      console.log('fetch studio')
  ...
  const artPiece = artPieces.find( piece => piece.id == artPieceId )

  const currentArtistPath = routing.urlForModel("artist", artist);
  const hasArtistProfile = Boolean(artist.profile?.images?.small)
  const hasAddress = artist.address || studio.address;

  console.log({artist, artPieces, studio})

  const isCurrent = (id) =>   artPieceId == id;
  ...
  return (
    ...
    <div className="art-piece-browser">
      ...
      <div className="pure-g alpha omega">
        <div
          className="pure-u-1-1 pure-u-sm-1-3 pure-u-md-1-4 pure-u-lg-1-5 art-piece__info"
        >
          <section className="artist__image">
            <div className="artist-profile__image">
              <a href={currentArtistPath}>
                { hasArtistProfile ?
                  <img
                    className="pure-img profile"
                    src={profileImagePath} /> : <i
          ...
          <section className="desc">
            { Boolean(artPiece.dimensions) &&
              <div className="desc__item">
                <h4 className="art-piece__info-title">
                  Dimensions
                </h4>
            ...
            { Boolean(artPiece.tags) &&
              <div className="desc__item">
                ...
                <div className="tags">
                  { artPiece.tags.map( tag => <ArtPieceTagLink tag={tag} key={tag.id} />) }
                </div>
              </div>
            }
            { ( Boolean(artPiece.displayPrice) || artPiece.sold ) && (
                <div className={cx('desc__item', {'desc__item--sold': artPiece.sold})} >
                  <h4 className="art-piece__info-title">
                    Price
                  </h4>
          ...
          (
            <div className="pure-u-1-1 art-piece__thumbs" >
              <div className="pure-g browser">
                ...
                return (
                <div key={piece.id}
                  className="piece pure-u-1-4 pure-u-sm-1-6 pure-u-md-1-8 pure-u-lg-1-10 padded-content"
                  className={cx({current: isCurrent(piece.id)})}
                >
                  <a
                    href="/art_pieces/{{ piece.id }}"
                    onClick="setCurrent(piece.id)"
                  >
                    <div
                      className="image"
                      style={{backgroundImage:`url(${piece.imageUrls.small})`}}
                    ></div>
                  </a>
                </div>
                )})}
