...
describe("FavoriteThis", () => {
  ...
  describe("default", function () {
    beforeEach(() => {
      mockApi.users.whoami.mockResolvedValue( { currentUser: { slug: "the-artist" }})
    ...
    it("sets up the directive with the art piece attributes", async function () {
      ...
      await waitFor(() => {
        ...
        expect(mockApi.favorites.add).toHaveBeenCalledWith("the-artist", "Artist", 10);
      });
    });
  ...
  describe("when you're not logged in", function () {
    ...
    it("sets up the directive with the art piece attributes", async function () {
      ...
      await waitFor(() => {
        expect(mockApi.users.whoami).toHaveBeenCalled();
        expect(screen.queryByText("You need to login", {exact: false})).toBeInTheDocument();
      });
    });
  });
  ...
  describe("when the favorite call fails", function () {
    ...
    it("sets up the directive with the art piece attributes", async function () {
      ...
      await waitFor(() => {
        expect(mockApi.users.whoami).toHaveBeenCalled();
        expect(screen.queryByText("That item doesn't seem to be available to favorite", {exact: false})).toBeInTheDocument();
      });
    });
  });
