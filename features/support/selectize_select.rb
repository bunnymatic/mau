...
"#{value}")').click(); }
    page.execute_script %{ #{find_selectized_control_js(key).strip}.find('.selectize-input').click(); }
    page.execute_script %{ #{find_selectized_control_js(key).strip}.find('.selectize-dropdown-content .option:contains("#{value}")').click(); }
  end
